...
import { bigint, varchar, tinyint, decimal, timestamp, date, text, mysqlTable, mysqlEnum, index, check, } from "drizzle-orm/mysql-core";
...
export const goals = mysqlTable("goals", {
    ...
    amountsCheck: check("chk_goals_amounts", sql `target_amount > 0 AND current_saved >= 0 AND monthly_contribution >= 0`),
    // 1% tolerance để tránh lỗi rounding khi tính completion
    savedLteTarget: check("chk_goals_saved_lte_target", sql `current_saved <= target_amount * 1.01`),
}));
