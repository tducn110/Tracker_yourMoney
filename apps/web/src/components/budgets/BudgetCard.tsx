...
export function BudgetCard({
  ...
  const targetAmount = new Decimal(budget.targetAmount || budget.budget_limit || 0).toNumber();
  const spent = new Decimal(budget.spent || 0).toNumber();
  const left = new Decimal(budget.left || Math.max(0, targetAmount - spent)).toNumber();
  const percent = budget.percent || (targetAmount > 0 ? Math.min(100, Math.round((spent / targetAmount) * 100)) : 0);
  
  const categories = budget.categories || [];
  const periodType = budget.periodType || budget.period_type || 'monthly';
  const endDateStr = budget.endDate || budget.end_date || new Date().toISOString();
  const isAllCategories = budget.isAllCategories || budget.is_all_categories || false;
  ...
  return (
    <div
      ...
      onDrop={onDrop}
      onDragEnd={onDragEnd}
      onClick={() => !reorderMode && router.push(`/budgets/${budget.id}`)}
      className={`
