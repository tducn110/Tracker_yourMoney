...
export default function BillsPage() {
  ...
  const totalMonthly = activeBills
    .reduce((sum: Decimal, b: Bill) => sum.plus(new Decimal(b.amount || 0)), new Decimal(0));
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="bg-linear-to-br from-amber-50 to-orange-50 rounded-xl border border-amber-200 p-5">
              ...
              <p className="text-2xl font-bold text-amber-700">{formatCurrency(totalMonthly.toNumber())}</p>
            </div>
                    ...
                    >
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-xl bg-amber-100 flex items-center justify-center text-xl">
                          {bill.icon || <Receipt size={20} className="text-amber-600" />}
                        </div>
                        <div>
                          ...
                          <div className="flex items-center gap-4 mt-1">
                            ...
                            <span className="text-xs px-2 py-0.5 rounded bg-blue-100 text-blue-700 font-medium capitalize">
                              {bill.frequency === 'monthly' ? 'Hàng tháng' : bill.frequency}
                            </span>
                          </div>
                        </div>
                      </div>
                      <div className="text-right">
                        ...
                        >
                          <Check size={12} className="mr-1 group-hover:scale-110 transition-transform" />
