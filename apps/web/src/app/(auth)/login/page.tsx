    ...
    <div className="min-h-screen flex" style={{ background: '#F8FAFC', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-md">
          ...
          <h2 className="text-[28px] font-bold mb-2" style={{ color: '#1E293B' }}>Chào mừng bạn! 👋</h2>
          ...
          <div className="space-y-3">
            <button
              ...
              className="w-full flex items-center justify-center gap-3 py-3.5 rounded-xl border transition-all hover:bg-slate-50 disabled:opacity-70 shadow-sm"
              style={{ borderColor: '#E2E8F0', background: '#FFFFFF', color: '#1E293B' }}
            >
              <img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" alt="Google" className="w-5 h-5" />
              <span className="font-semibold text-[15px]">Tiếp tục với Google</span>
            </button>

            <button
              onClick={() => handleLogin('facebook', loginWithFacebook)}
              disabled={!!loading || authLoading}
              className="w-full flex items-center justify-center gap-3 py-3.5 rounded-xl transition-all hover:opacity-90 disabled:opacity-70 shadow-sm"
              style={{ background: '#1877F2', color: '#FFFFFF' }}
            >
              <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
              <span className="font-semibold text-[15px]">Tiếp tục với Facebook</span>
            </button>
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                disabled={!!loading || authLoading}
                className="flex items-center justify-center gap-2 py-3 rounded-xl transition-all hover:opacity-90 disabled:opacity-70 shadow-sm"
              ...
              >
                <svg className="w-5 h-5 fill-current" viewBox="0 0 24 24"><path d="M17.05 20.28c-.96.95-2.44 2.08-4.4 2.08-2.25 0-3.11-1.39-5.22-1.39-2.14 0-3.22 1.35-5.18 1.35-1.74 0-3.32-1.35-4.24-2.5-2.02-2.52-2.6-6.52-1.44-8.82 1.15-2.29 3.48-3.72 5.56-3.72 1.58 0 2.97.94 4.02.94.98 0 2.06-.94 3.9-.94 1.83 0 3.32.78 4.28 2.08-2.31 1.34-1.92 4.74.4 5.92-.8 2.3-2.14 4.05-3.48 5.43v-.43zm-4.32-15.65c.98-1.22 1.35-2.88 1.05-4.52-1.48.06-2.92.93-3.78 2.05-1.02 1.25-1.44 2.96-1.14 4.54 1.58.12 3.01-.85 3.87-2.07z"/></svg>
            ...
            <p className="text-[13px] text-center leading-relaxed" style={{ color: '#64748B' }}>
              Bằng cách đăng nhập, bạn đồng ý với <a href="#" className="underline">Điều khoản dịch vụ</a> và <a href="#" className="underline">Chính sách bảo mật</a> của chúng tôi.
            </p>
          </div>

          {/* Feature hint */}
          <div className="mt-12 p-4 rounded-2xl border border-blue-50" style={{ background: '#F0F7FF' }}>
