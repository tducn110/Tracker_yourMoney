...
## ⚠️ KIẾN TRÚC THAY ĐỔI — ĐỌC TRƯỚC

### Những gì đã bị XÓA hoàn toàn khỏi tầng Database:

| Loại | Tên | Lý do xóa | Chuyển về đâu |
|------|-----|-----------|---------------|
| ❌ View | `v_monthly_summary` | Business logic trong DB vi phạm Clean Architecture | `packages/db/queries/summary.ts` |
| ❌ View | `v_safe_to_spend` | S2S formula thuộc về API layer | `apps/api/services/s2s-engine.ts` |
| ❌ View | `v_category_spending_current` | Query logic thuộc về API layer | `packages/db/queries/transactions.ts` |
| ❌ View | `v_upcoming_bills` | Query logic thuộc về API layer | `packages/db/queries/bills.ts` |
| ❌ Stored Proc | `sp_generate_bill_payments_for_month` | Workflow logic thuộc về API layer | `apps/api/services/bill-service.ts` |
| ❌ Stored Proc | `sp_cash_wallet_sync` | Transaction + side-effect logic | `apps/api/services/wallet-service.ts` |
| ❌ Trigger | `trg_after_user_insert` | Side effect ẩn, khó debug | `apps/api/services/user-service.ts` |
| ❌ Trigger | `trg_goals_auto_complete` | Business rule phải test được | `apps/api/services/goal-service.ts` |
| ❌ Trigger | `trg_audit_transaction_delete` | Application-level logging | `apps/api/middleware/audit.ts` |

### Database layer CHỈ giữ:
✅ Table definitions (CREATE TABLE)
✅ Foreign Key constraints
✅ Indexes (performance)
✅ Check constraints (data integrity cơ bản: amount > 0)
✅ Drizzle ORM schema types

### v11.1 — Các lỗi cấu trúc dữ liệu đã vá (Core Schema Audit):

| # | Bảng | Lỗi cũ | Bản vá |
|---|------|---------|--------|
| 1 | `transactions` | Thiếu `currency_code` + `base_amount` → lịch sử nát khi user đổi currency | **→ v12.0: defer Phase 2 (VND-only MVP)** |
| 2 | `transactions` | `transaction_date DATE` → timezone ambiguity | Đổi thành `executed_at TIMESTAMP` **→ v12.0: refine thành `display_date DATE`** |
| 3 | `bill_payments` | `UNIQUE KEY uq_bill_period` → chặn thanh toán từng phần | Xóa Unique Key; mỗi row = 1 payment event; trạng thái tính `SUM` tại API ✅ |
| 4 | Drizzle schemas | `bigint({ mode: "number" })` → JS overflow với TiDB distributed ID | Đổi toàn bộ sang `mode: "string"` ✅ |

### v12.0 — Merge từ V12 Lite Spec (delta analysis):

| # | Thay đổi | v11.1 | v12.0 | Lý do chọn v12.0 |
|---|----------|-------|-------|------------------|
| A | `transactions` date | `executed_at TIMESTAMP` | **`display_date DATE` + `created_at` (system)** | Pure Tracker: user tự chọn ngày hiển thị (có thể log quá khứ). DATE đúng ngữ nghĩa, không timezone confusion vì user kiểm soát nó. `created_at` làm audit trail. |
| B | `transactions` currency | 3 cột `currency_code`, `exchange_rate`, `base_amount` | **Removed → Phase 2** | MVP = VND-only. 3 cột thêm complexity không cần thiết ở scale này. Migrate khi expand. |
| C | `transactions.type` | `ENUM('income','expense')` | **Thêm `'transfer'`** | Transfer ví ↔ tài khoản là use case thực tế. |
| D | `cash_wallet` | `balance` only | **Thêm `initial_balance`** | `initial_balance` = snapshot khi onboard. Net delta = `balance − initial_balance`. Cần cho báo cáo. |
| E | Zod schemas | Không có trong doc | **Thêm section mới** | Source of truth cho API layer. |
| F | DB init code | Chỉ có connection string | **Thêm `packages/db/src/index.ts`** | Boilerplate chuẩn, không viết lại mỗi service. |

> **Giữ nguyên từ v11.1 (V12.0 Lite quá giản lược):**
> - `goals.status` = 4 giá trị `(active, completed, paused, cancelled)` — *paused/cancelled là trạng thái tài chính thực tế*
> - `users` table = đầy đủ fields (username, avatar_url, is_active, email_verified) — *cần cho Auth flow*
> - 12 bảng đầy đủ — *phân loại rõ Phase 1 / Phase 2 thay vì xóa*
...
```typescript
import {
  bigint, char, decimal, varchar, timestamp,
  mysqlTable, index
} from "drizzle-orm/mysql-core";
import { users } from "./users";
...
```typescript
import {
  bigint, varchar, decimal, tinyint, timestamp, date,
  mysqlTable, mysqlEnum, index
} from "drizzle-orm/mysql-core";
import { users } from "./users";
...
export const goals = mysqlTable("goals", {
  ...
  currentSaved:        decimal("current_saved", { precision: 15, scale: 2 }).notNull().default("0"),
  monthlyContribution: decimal("monthly_contribution", { precision: 15, scale: 2 }).notNull().default("0"),
...
```typescript
import {
  bigint, decimal, timestamp, mysqlTable
} from "drizzle-orm/mysql-core";
import { users } from "./users";
...
export const insertUserSchema = z.object({
  ...
  username:     z.string().min(3).max(50).regex(/^[a-z0-9_]+$/, "Chỉ dùng chữ thường, số, gạch dưới"),
});
...
export const insertTransactionSchema = z.object({
  ...
  displayDate: z.coerce.date().transform((d) =>
    d.toISOString().split("T")[0]  // → "2026-04-01" (DATE string cho Drizzle)
  ),
...
*File này là tài liệu database của **S2S Finance — Finance Tracker V3 (Antigravity V1.2 Protocol)** — v11.1. Chỉ chứa Table Definitions, Foreign Keys, và Indexes. Business logic đã được tách hoàn toàn ra khỏi tầng Database và nằm tại `apps/api/src/services/`.*

*v11.1 fixes: [#3] Xóa UNIQUE uq_bill_period → partial payments OK | [#4] Drizzle bigint mode:"string" → TiDB overflow-safe.*
*v12.0 merge: [A] display_date DATE + created_at (dual-date Pure Tracker design) | [B] Currency fields → Phase 2 | [C] type enum + 'transfer' | [D] cash_wallet.initial_balance | [E] Zod schemas | [F] DB connection boilerplate.*
