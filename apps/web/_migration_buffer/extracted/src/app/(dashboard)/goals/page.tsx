...
import { formatVND, mockGoals } from '../../data/mockData';
...
export default function GoalsPage() {
  ...
  const handleGoalClick = (goal: typeof mockGoals[0]) => {
  ...
  return (
    <div className="p-6 space-y-6">
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-500">Mục tiêu: {formatVND(goal.targetAmount)}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-bold text-gray-800">{formatVND(goal.currentSaved)}</span>
                </div>

                <div className="mt-3 pt-3 border-t border-gray-100">
                  <div className="flex items-center gap-2 text-xs text-gray-500">
                    <TrendingUp size={14} />
                    <span>+{formatVND(goal.monthlyContribution)}/tháng</span>
                  </div>
                </div>
              </motion.div>
            );
      ...
      {completedGoals.length > 0 && (
        <div>
          <h2 className="text-lg font-bold text-gray-800 mb-4">Đã hoàn thành ({completedGoals.length})</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {completedGoals.map((goal) => (
              <div
                key={goal.id}
                className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl border border-green-200 p-6 relative overflow-hidden"
              >
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <h3 className="font-bold text-gray-800">{goal.name}</h3>
                    <p className="text-xs text-green-700">Hoàn thành {goal.completedAt}</p>
                  </div>
                </div>
                <div className="text-sm text-gray-700">
                  <p>Mục tiêu: <span className="font-bold">{formatVND(goal.targetAmount)}</span></p>
                </div>
              </div>
            ))}
          </div>
