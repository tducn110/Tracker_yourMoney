...
import {
  ...
  ArrowUpDown, Download, ReceiptText,
} from 'lucide-react';
  ...
  return (
    <div className="grid grid-cols-3 gap-3">
      ...
      <div className="bg-emerald-50 border border-emerald-100 rounded-2xl px-4 py-3">
        ...
        <p className="text-[18px] font-black text-emerald-700 leading-none">{formatVND(income)}</p>
      </div>
      ...
      <div className="bg-red-50 border border-red-100 rounded-2xl px-4 py-3">
        ...
        <p className="text-[18px] font-black text-red-600 leading-none">{formatVND(expense)}</p>
      </div>
      ...
      >
        <p className="text-[10px] font-black uppercase tracking-wide mb-1" style={{ color: net >= 0 ? '#2563eb' : '#dc2626' }}>
          {net >= 0 ? 'Dư ra' : 'Bội chi'}
        </p>
        <p className="text-[18px] font-black leading-none" style={{ color: net >= 0 ? '#2563eb' : '#dc2626' }}>
  ...
  return (
    <div className="flex items-center gap-4 px-5 py-3.5 hover:bg-gray-50/80 cursor-pointer transition-colors group border-b border-gray-50 last:border-0">
      ...
      <div className="hidden sm:block flex-shrink-0 text-center">
        <p className="text-[11px] font-bold text-gray-500">{tx.date}</p>
      </div>
...
export default function TransactionsPage() {
  ...
  const filtered = useMemo(() => {
    let list = mockTransactions.filter((tx) => {
      const q  = search.toLowerCase();
      const ok = !q || tx.note.toLowerCase().includes(q) || tx.category.toLowerCase().includes(q);
      return ok && (filter === 'all' || tx.type === filter);
    });

    if (sortOrder === 'oldest') list = [...list].reverse();
    if (sortOrder === 'amount_desc') list = [...list].sort((a, b) => Math.abs(b.amount) - Math.abs(a.amount));
    if (sortOrder === 'amount_asc')  list = [...list].sort((a, b) => Math.abs(a.amount) - Math.abs(b.amount));
  ...
  return (
    <div className="p-4 md:p-6 pb-24 max-w-[960px] mx-auto space-y-5">
      ...
      <div>
        ...
        <p className="text-[12px] font-bold text-gray-400 mt-0.5">Tháng 4/2026 · {mockTransactions.length} giao dịch</p>
      </div>
      ...
      <div className="flex items-center justify-between">
        <p className="text-[11px] font-bold text-gray-400">
          Hiển thị {filtered.length} / {mockTransactions.length} giao dịch
        </p>
        {filtered.length < mockTransactions.length && (
