...
export function InlineChatQuickAdd({ onSubmit }: InlineChatQuickAddProps) {
  ...
  const handleConfirm = async (msg: Message) => {
    ...
    try {
      ...
      const confirmMsg: Message = {
        id: `confirm-${Date.now()}`,
        role: 'bot',
        content: `✅ Đã lưu **${msg.data!.note}** — **${formatVND(msg.data!.amount)}** vào danh sách giao dịch!`,
        timestamp: new Date(),
      };
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm">
      ...
      {/* ── Header ── */}
      <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100 bg-gradient-to-r from-blue-600 to-indigo-600">
        ...
        <AnimatePresence initial={false}>
          {messages.map((msg) => (
            ...
            >
              {/* Avatar */}
              <div className={`
                w-7 h-7 rounded-xl flex-shrink-0 flex items-center justify-center mt-0.5
                ${msg.role === 'user'
        ...
        <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
          {QUICK_CHIPS.map(({ label, icon: Icon }) => (
            <button
              ...
              disabled={isTyping}
              className="flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-full text-[12px] font-black transition-all hover:scale-105 disabled:opacity-40"
            >
              <Icon size={11} />
              {label}
            </button>
