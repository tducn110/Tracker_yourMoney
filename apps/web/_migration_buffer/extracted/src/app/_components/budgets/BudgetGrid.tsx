...
import { mockBudgets, mockBudgetSummary, formatVND } from '@/app/data/mockData';
...
function FeaturedBudgetCard({ budget }: { budget: typeof mockBudgets[0] }) {
  const navigate = useNavigate();
  const status = getStatusBadge(budget.percent);
  const progressColor = getProgressColor(budget.percent);
  const amountColor = budget.left < 0 ? '#dc2626' : '#111827';
  ...
  return (
    <div
      ...
      className="group bg-white border border-gray-200 rounded-2xl p-5 shadow-sm hover:shadow-md hover:border-gray-300 transition-all duration-200 cursor-pointer"
    >
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1 flex-shrink-0">
            {budget.categories.slice(0, 2).map((cat) => (
              <div
                key={cat.id}
                className="w-10 h-10 rounded-xl bg-amber-50 flex items-center justify-center text-[20px]"
              >
                {cat.icon}
              </div>
            ))}
          ...
          <div className="flex-1 min-w-0">
            <h3
              ...
              style={{ color: budget.percent >= 100 ? '#dc2626' : '#111827' }}
            >
              {budget.name}
            </h3>
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className="text-[10px] font-bold text-gray-400 flex items-center gap-1">
                ...
                {periodLabel[budget.period_type]}
              </span>
              {daysLeft > 0 && (
        ...
        {budget.percent >= 80 && (
          <div className="flex-shrink-0 ml-2">
            <AlertCircle
              size={18}
              style={{ color: budget.percent >= 100 ? '#ef4444' : '#f59e0b' }}
            />
          </div>
        )}
      </div>
      ...
      <div className="flex items-end justify-between mb-4">
        <div>
          <div className="flex items-baseline gap-1.5">
            ...
            >
              {budget.left < 0 ? '−' : ''}{formatVND(Math.abs(budget.left))}
            </span>
            <span className="text-[13px] font-bold text-gray-400">còn lại</span>
          </div>
          <p className="text-[12px] font-bold text-gray-500 mt-1">
            <span style={{ color: progressColor }}>{formatVND(budget.spent)}</span>
            {' '}/ {formatVND(budget.budget_limit)} hạn mức
          </p>
        </div>
      ...
      <div>
        <div className="h-3 w-full bg-gray-100 rounded-full overflow-hidden">
          <div
            className="h-full rounded-full transition-all duration-700"
            style={{ width: `${budget.percent}%`, backgroundColor: progressColor }}
          />
        </div>
      </div>
...
function BudgetCreateCTA() {
  ...
  return (
    <button
      ...
      className="w-full bg-white border border-gray-200 rounded-2xl p-5 shadow-sm hover:shadow-md hover:border-gray-200 transition-all duration-200 cursor-pointer group"
    >
      <div className="flex items-center justify-center gap-3">
        <div className="w-7 h-7 rounded-lg bg-gray-50 flex items-center justify-center">
          <Layers size={14} className="text-gray-400 group-hover:text-blue-500 transition-colors" />
        </div>
        <div className="text-center">
          <p className="text-[14px] font-black text-gray-900 group-hover:text-blue-600 transition-colors">
            Tạo ngân sách mới
          </p>
          <p className="text-[10px] font-bold text-gray-300 mt-0.5">Quản lý chi tiêu thông minh</p>
        </div>
...
export function BudgetGrid() {
  ...
  // Lấy budget mới nhất (id lớn nhất trong active, không phải all-categories)
  const activeBudgets = mockBudgets.filter((b) => b.status === 'active' && !b.is_all_categories);
  const latestBudget = activeBudgets[activeBudgets.length - 1] ?? null;
