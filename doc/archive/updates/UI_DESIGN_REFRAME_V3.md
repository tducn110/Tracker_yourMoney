...
Finance Tracker V3 chuyển mình từ một công cụ "ghi chép chi tiêu" thuần túy sang hệ thống **"Quản lý Ngân sách Chiến lược"**. Thay vì chỉ nhìn vào con số Budget-First duy nhất, người dùng giờ đây được trao quyền kiểm soát thông qua các Ngân sách (Budgets) linh hoạt.
...
## 2. 🔄 So sánh Cũ (Legacy Budget) vs Mới (V3 Budget-First)

| Đặc điểm | Hệ thống cũ (Budget Legacy) | Hệ thống mới (V3 Budget-First) |
|----------|--------------------------|---------------------------------|
| **Trọng tâm** | Chỉ số Budget (Một con số duy nhất) | Đa ngân sách (Ngân sách tổng + Ngân sách hạng mục) |
...
3.  **Wallet Management**:
    ...
    *   *Yêu cầu*: Cần thêm metadata để hỗ trợ logic "Không tính vào Budget" cho ví tiền mặt (Cash Wallet) như trong UI mock-up.
